...
iso_label="Tokumei${iso_mainver//.}"
iso_publisher="Tokumei <https://tokumeios.github.io>"
iso_application="Tokumei"
install_dir=tokumeicd
work_dir=work
out_dir=out
gpg_key=
arch="$(uname -m)"
...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts -g /boot/tokumeicd.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_boot() {
    ...
    cp ${work_dir}/${arch}/airootfs/boot/tokumeicd.img ${work_dir}/iso/${install_dir}/boot/${arch}/tokumeicd.img
